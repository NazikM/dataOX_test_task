...
class TorontoSpider(scrapy.Spider):
    name = 'real_estate_toronto'
    allowed_domains = ['www.kijiji.ca']
    # start_urls = ['https://www.kijiji.ca/b-apartments-condos/city-of-toronto/']
    start_url = 'https://www.kijiji.ca'
    ...
    def start_requests(self):
        ...
        # return [scrapy.Request(url='https://www.kijiji.ca/b-apartments-condos/city-of-toronto/c37l1700273', headers={
        #     'User-Agent': choice(self.user_agents)
        # }, dont_filter=True, callback=self.parse)]

    def parse(self, response):
        estates = response.xpath("//div[@class='info-container']/div[@class='title']/a/@href").getall()
        for estate_url in estates:
            yield scrapy.Request(url=self.start_url + estate_url, callback=self.parse_item,
